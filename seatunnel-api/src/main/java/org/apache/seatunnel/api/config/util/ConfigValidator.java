...
public class ConfigValidator {
    ...
    public void validate(OptionRule rule) {
        ...
        for (RequiredOption requiredOption : requiredOptions) {
            ...
            for (ConfigEntry<?> option : requiredOption.getOptions()) {
                if (SingleChoiceConfigEntry.class.isAssignableFrom(option.getClass())) {
        ...
        for (ConfigEntry option : rule.getOptionalOptions()) {
            if (SingleChoiceConfigEntry.class.isAssignableFrom(option.getClass())) {
                validateSingleChoice(option);
            }
        }
    ...
    void validateSingleChoice(ConfigEntry option) {
    ...
    private List<ConfigEntry<?>> getAbsentOptions(List<ConfigEntry<?>> requiredOption) {
        List<ConfigEntry<?>> absent = new ArrayList<>();
        for (ConfigEntry<?> option : requiredOption) {
    ...
    void validate(RequiredOption.AbsolutelyRequiredOptions requiredOption) {
        List<ConfigEntry<?>> absentOptions = getAbsentOptions(requiredOption.getRequiredOption());
    ...
    boolean hasOption(ConfigEntry<?> option) {
        return config.getOptional(option).isPresent();
    }

    boolean validate(RequiredOption.BundledRequiredOptions bundledRequiredOptions) {
        List<ConfigEntry<?>> bundledOptions = bundledRequiredOptions.getRequiredOption();
        List<ConfigEntry<?>> present = new ArrayList<>();
        List<ConfigEntry<?>> absent = new ArrayList<>();
        for (ConfigEntry<?> option : bundledOptions) {
            if (hasOption(option)) {
                present.add(option);
            } else {
                absent.add(option);
        ...
        throw new OptionValidationException(
                ...
                OptionUtil.getOptionKeys(bundledOptions),
                OptionUtil.getOptionKeys(present),
                OptionUtil.getOptionKeys(absent));
    }

    void validate(RequiredOption.ExclusiveRequiredOptions exclusiveRequiredOptions) {
        List<ConfigEntry<?>> presentOptions = new ArrayList<>();

        for (ConfigEntry<?> option : exclusiveRequiredOptions.getExclusiveOptions()) {
            if (hasOption(option)) {
    ...
    void validate(RequiredOption.ConditionalRequiredOptions conditionalRequiredOptions) {
        ...
        List<ConfigEntry<?>> absentOptions =
    ...
    private <T> boolean validate(Condition<T> condition) {
        ConfigEntry<T> option = condition.getOption();
